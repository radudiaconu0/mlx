    ...
    MLX_SWITCH_ALL_TYPES(in.dtype(), CTYPE_IN, [&]() {
      MLX_SWITCH_ALL_TYPES(out.dtype(), CTYPE_OUT, [&]() {
        ...
        if constexpr (std::is_convertible_v<InType, OutType>) {
          auto in_ptr =
              rocthrust::device_pointer_cast(in.data<InType>() + inp_offset);
          auto out_ptr =
              rocthrust::device_pointer_cast(out.data<OutType>() + out_offset);
          if (ctype == CopyType::Scalar) {
            rocthrust::copy_n(
                policy,
                mxrocm::make_repeat_iterator(in_ptr),
                out.data_size(),
                out_ptr);
          } else if (ctype == CopyType::Vector) {
            rocthrust::copy_n(policy, in_ptr, out.data_size(), out_ptr);
          } else {
            bool dynamic = dynamic_i_offset || dynamic_o_offset;
            if (dynamic) {
              throw std::runtime_error(
                  "Dynamic copy not implemented for ROCm backend.");
            }
            ...
            if (ctype == CopyType::General) {
              rocthrust::copy_n(
                  policy,
                  mxrocm::make_general_iterator<int64_t>(
    ...
    MLX_SWITCH_ALL_TYPES(val.dtype(), CTYPE_IN, [&]() {
      MLX_SWITCH_ALL_TYPES(out.dtype(), CTYPE_OUT, [&]() {
        ...
        if constexpr (std::is_convertible_v<InType, OutType>) {
          rocthrust::copy_n(
              policy,
              mxrocm::make_repeat_iterator(
                  rocthrust::device_pointer_cast(val.data<InType>())),
              out.data_size(),
